...
#
class OutlineFilter < Nanoc3::Filter
  include ERB::Util
  ...
  #
  def run(content, params = {:numbering => true, :numbering_start => 1, :toc_style => 'ol', :toc_range => 'h1-h6'})
    @str = content
    ...
    @numbering = params[:numbering].nil? ? true : params[:numbering]
    @numbering_start = params[:numbering_start] || 1
    @toc_style = params[:toc_style] || 'ol'
    @toc_range = params[:toc_range] || 'h1-h6'
    @title = params[:title]
...
#    (params[:title].nil? ? "" : "<h2>#{params[:title]}</h2>\n\n") +
    doc.to_html
  end
